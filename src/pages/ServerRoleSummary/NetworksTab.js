...
class NetworksTab extends Component {
  ...
  handleEditNetwork = (idx, data) => {
    if(this.state.mode === MODE.NONE) {
      this.setState({mode: MODE.EDIT, editIdx: idx, networkName: data.get('name')});
    }
  }
  ...
  renderNetworkTable() {
    ...
    const rows = this.props.model.getIn(['inputModel','networks'])
      ...
      .map((network, idx) => {
        let theProps = {};
        if(this.state.mode === MODE.EDIT && idx === this.state.editIdx) {
          theProps.className = 'edit-selected';
        }
        return (
          <tr {...theProps} key={idx}>
